...
/// Pricing for constant Bls12 operations (ADD and MUL in G1 and G2, as well as mappings)
#[derive(Debug, Copy, Clone)]
pub struct Bls12MultiexpPricerWithTableG1<P: PointScalarLength> {
    /// Base const of the operation (G1 or G2 multiplication)
    pub base_price: Bls12ConstOperations,
    _marker: std::marker::PhantomData<P>,
}
#[derive(Debug, Copy, Clone)]
pub struct Bls12MultiexpPricerWithTableG2<P: PointScalarLength> {
    /// Base const of the operation (G1 or G2 multiplication)
    pub base_price: Bls12ConstOperations,
    _marker: std::marker::PhantomData<P>,
}
...
impl<P: PointScalarLength> Pricer for Bls12MultiexpPricerWithTableG1<P> {
    fn cost(&self, input: &[u8]) -> U256 {
        ...
        let discount = if num_pairs > BLS12_MULTIEXP_PAIRS_FOR_MAX_DISCOUNT {
            ...
            let table_entry = BLS12_MULTIEXP_DISCOUNTS_TABLE_G1[num_pairs - 1];
            table_entry[1]
        };
...
impl<P: PointScalarLength> Pricer for Bls12MultiexpPricerWithTableG2<P> {
    fn cost(&self, input: &[u8]) -> U256 {
        let num_pairs = input.len() / P::LENGTH;
        if num_pairs == 0 {
            return U256::zero();
        ...
        let discount = if num_pairs > BLS12_MULTIEXP_PAIRS_FOR_MAX_DISCOUNT {
            ...
            let table_entry = BLS12_MULTIEXP_DISCOUNTS_TABLE_G2[num_pairs - 1];
            table_entry[1]
        };
        U256::from(self.base_price.price) * U256::from(num_pairs) * U256::from(discount)
            / U256::from(BLS12_MULTIEXP_DISCOUNT_DIVISOR)
...
pub type Bls12MultiexpPricerG1 = Bls12MultiexpPricerWithTableG1<G1Marker>;

/// Multiexp pricer in G2
pub type Bls12MultiexpPricerG2 = Bls12MultiexpPricerWithTableG2<G2Marker>;
